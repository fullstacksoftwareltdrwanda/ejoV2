...
import {
    ShieldAlert, Search, Loader2, ArrowRight, Trophy,
...
function markColor(m: number) {
    if (m >= 35) return { bg: "bg-emerald-100", text: "text-emerald-700", bar: "bg-emerald-500" };
    if (m >= 25) return { bg: "bg-amber-100", text: "text-amber-700", bar: "bg-amber-400" };
...
function MarkBadge({ marks }: { marks: number }) {
    ...
    return (
        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-widest ${c.bg} ${c.text}`}>
            {marks >= 35 ? <Star className="w-3 h-3" /> : marks >= 25 ? <TrendingUp className="w-3 h-3 text-amber-500" /> : <TrendingDown className="w-3 h-3" />}
            {marks} / 40
        </span>
    );
}
...
export default function DisciplineMarksPage() {
    ...
    async function fetchData() {
        try {
            ...
            setData(d);
            // Pre-load local marks state for all students
    ...
    async function saveMark(studentId: string) {
        ...
        try {
            ...
            toast.success("Discipline mark saved!");
            await fetchData();
        } catch {
    ...
    if (loading) return (
        <div className="h-[60vh] flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Loading conduct records...</p>
        </div>
    );
    ...
    const concernStudents = [...allStudents]
        ...
        <div className="space-y-10 animate-fade-up">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="relative">
                    ...
                    <input
                        className="bg-white border border-slate-100 rounded-2xl pl-10 pr-6 py-4 text-xs font-semibold outline-none focus:ring-4 focus:ring-emerald-500/10 shadow-sm w-72"
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className={`rounded-[2rem] p-6 ${markColor(schoolAvg).bg}`}>
                    <ShieldAlert className={`w-5 h-5 mb-3 ${markColor(schoolAvg).text}`} />
                    <p className={`text-3xl font-black ${markColor(schoolAvg).text}`}>{data?.schoolAverage}</p>
                ...
                <div className="bg-red-50 rounded-[2rem] p-6 border border-red-100">
                    ...
                    <p className="text-3xl font-black text-red-600">{concernStudents.length}</p>
                    <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mt-1">At Risk (&lt;25)</p>
                </div>
            </div>

            {/* Best + At-risk students */}
                ...
                <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 space-y-5">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Top students this period</p>
                        </div>
                    </div>
                    <div className="space-y-3">
                        {bestStudents.map((s, idx) => (
                            <div key={s.id} className="flex items-center gap-4 p-3 rounded-2xl bg-emerald-50/50 border border-emerald-50">
                                <div className="w-8 h-8 bg-emerald-600 rounded-xl flex items-center justify-center text-white text-[11px] font-black shrink-0">
                                    {idx === 0 ? "🥇" : idx === 1 ? "🥈" : "🥉"}
                                </div>
                                <div className="flex-grow">
                                    <p className="text-sm font-black text-slate-900 uppercase tracking-tight">{s.firstName} {s.lastName}</p>
                        ...
                        {bestStudents.length === 0 && (
                            <p className="text-center py-6 text-[9px] font-black uppercase tracking-widest text-slate-300">No data yet</p>
                        )}
                    </div>
                </div>
                    ...
                    <div className="space-y-3 max-h-52 overflow-y-auto">
                        {concernStudents.length === 0 ? (
                            <div className="py-6 text-center">
                                <p className="text-[9px] font-black uppercase tracking-widest text-emerald-500">All students are doing well! ✓</p>
                            </div>
                        ) : concernStudents.map(s => (
                            <div key={s.id} className="flex items-center gap-4 p-3 rounded-2xl bg-red-50/50 border border-red-50">
                                <div className="w-8 h-8 bg-red-500 rounded-xl flex items-center justify-center text-white text-[11px] font-black shrink-0">
            ...
            <div className="space-y-4">
                <h2 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Class Breakdown — click to manage marks</h2>

                {filteredClasses.length === 0 ? (
                    <div className="py-24 bg-slate-50 rounded-[3rem] border border-dashed border-slate-200 flex flex-col items-center gap-4">
                        <ShieldAlert className="w-12 h-12 text-slate-200" />
                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">No classes match your search.</p>
                    </div>
                ) : filteredClasses.map((cls: any) => {
                    const avg = parseFloat(cls.averageDiscipline || "0");
                    ...
                    return (
                        <div key={cls.id} className="bg-white border border-slate-100 rounded-[2rem] shadow-sm overflow-hidden">
                            ...
                            >
                                ...
                                <div className="flex items-center gap-4">
                                    <div className="text-right hidden md:block">
                                        <p className={`text-xl font-black ${c.text}`}>{cls.averageDiscipline}</p>
                                        <p className="text-[8px] font-black uppercase tracking-widest text-slate-400">Class Average</p>
                                    </div>
                                    {/* Progress bar */}
                            ...
                            {isOpen && (
                                <div className="border-t border-slate-50 p-6 space-y-3 max-h-[420px] overflow-y-auto">
                                    {(cls.users || []).length === 0 ? (
                                        ...
                                        .map((student: any) => {
                                            const m = localMarks[student.id] ?? student.disciplineMarks ?? 40;
                                                ...
                                                <div key={student.id} className="bg-slate-50/50 rounded-2xl p-4 flex items-center gap-4 border border-slate-100 hover:bg-white hover:shadow-md transition-all">
                                                    ...
                                                    <div className="flex items-center gap-2 shrink-0">
                                                        <input
                                                            ...
                                                            className="w-16 bg-white border border-slate-200 rounded-xl px-2 py-2 text-xs font-black text-center outline-none focus:ring-2 focus:ring-emerald-400"
                                                        />
                                                        <button
